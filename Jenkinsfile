	...
	rtMaven.tool = 'M3'
	def buildInfo
	def ARTIFACTORY_LOCAL_SNAPSHOT_REPO = 'kmdevops'
	...
	pipeline {
		...
		stage('Build Container Image'){
			...
			steps{
				...
				// sh 'sudo docker run --rm -e "DD_URL=http://192.168.138.212:8080" -e "DD_API_KEY=59696caee6a62cb3eae309035367e661ec1e7768" -e "DD_PRODUCT_TYPE_NAME=Dev" -e "DD_PRODUCT_NAME=Trivy_Docker_Scan" \
				//    -e "DD_ENGAGEMENT_NAME=Docker_Scan" -e "DD_TEST_NAME=Trivy Scan" -e "DD_TEST_TYPE_NAME=Trivy Scan" -e "DD_FILE_NAME=trivy-scan-report-$BUILD_NUMBER.json" -v "${WORKSPACE}/trivy-scan-results/" maibornwolff/dd-import:latest dd-reimport-findings.sh'
				publishHTML (target : [
                    		     allowMissing: true,
		...
		stage('DAST Scan'){
			steps{
				...
				sh 'sudo docker run --name OWASP-Zap -t tactuumadmin/zap2docker-stable:latest zap-full-scan.py -t http://$QA_DOCKER_HOST:6090/ -I'
            		}
        	}
